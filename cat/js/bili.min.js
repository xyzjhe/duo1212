...
import {	config as def_conf} from "./bili_min_ext.js";
//import {	config as loc_conf} from "http://127.0.0.1:9978/file/tvbox/bili_min_local.js";
import {	config as loc_conf} from "./bili_min_local.js";
let siteKey = '';
let siteType = 0;
let config = {
...
async function init(cfg) {
	...
	if (!_.isEmpty(userID)) {
		...
		const resourceListUrl = 'https://api.bilibili.com/x/v3/fav/folder/created/list-all?up_mid=' + userID;
		const resourceList = JSON.parse(await request(resourceListUrl, getHeaders())).data.list;
		for (const ff of resourceList) {
			resourceListID.push(ff.id);
			resourceCount.push(ff.media_count);
		};
	};
	const sh = {
		type_name: "⌛稍后再看",
...
async function category(tid, page, filter, ext) {
	...
	try {
		...
		if (tid == "首页") {
			...
			url = "https://api.bilibili.com/x/v3/fav/resource/list?media_id=" + resourceListID[page - 1] + "&ps=" + resourceCount[page - 1];
		} else if (tid == "稍后再看") {
		...
		for (const item of items) {
			...
			if (tid == "收藏") {
				...
				const time = turnYMD(item.fav_time);
				video.vod_remarks = title + ': ' + time;
			} else {
		...
		if (tid == "排行榜" || tid == "入站必刷" || tid == "稍后再看") {
			const result = {
				page: page,
				pagecount: data.numPages ? ? page,
				limit: videos.length,
...
async function detail(ids) {
	try {
		...
		const cid = detailData.cid;
		const mid = detailData.owner.mid;
		const ownerUrl = 'https://api.bilibili.com/x/relation/stat?vmid=' + mid + '&jsonp=jsonp';
		const follower = JSON.parse(await request(ownerUrl, getHeaders())).data.follower;
		const onlineUrl = 'https://api.bilibili.com/x/player/online/total?aid=' + aid + '&cid=' + cid;
		const online = JSON.parse(await request(onlineUrl, getHeaders())).data.total;
		const video = {
			vod_id: bvid,
			...
			vod_actor: '',
			vod_director: detailData.owner.name + '&nbsp &nbsp ' + '粉丝数：' + turnNum(follower),
			vod_content: '点赞:' + like + '-投币:' + coin + '-收藏:' + favorite + '-转发:' + share + '\n' + detailData.desc,
		};
		const playurldata = 'https://api.bilibili.com/x/player/playurl?avid=' + aid + '&cid=' + cid + '&qn=127&fnval=4048&fourk=1';

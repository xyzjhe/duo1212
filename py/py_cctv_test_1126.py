...
class Spider(Spider):  # 元类 默认的元类 type
		...
		try:
			if tid=="搜索":
				...
				if len(videoList)<1:
					...
					fromId='央视'
					if tid=="电视剧" or tid=="纪录片" or tid=='搜索栏目':
						...
						videoList=self.get_EpisodesList_re(htmlTxt=htmlTxt,patternTxt=patternTxt[3])
					if len(videoList)<1:
	...
	def searchContent(self,key,quick):
		...
		htmlTxt=self.webReadFile(urlStr='https://search.cctv.com/search.php?qtext={0}&type=video'.format(key),header=self.header)
		videos=self.get_list_search_column(html=htmlTxt,tid='搜索栏目')
		# htmlTxt=self.webReadFile(urlStr=Url,header=self.header)
	...
	def playerContent(self,flag,id,vipFlags):
		...
		if flag=='CCTV':
			...
			try:
				...
				if html.find('window.location.href')>5:
					url=self.get_RegexGetText(Text=html,RegexText=r"href\s*=\s*(\"|')(.+?)(\"|')",Index=2)
	...
	def get_list(self,html,tid):
		...
		return videos
	#分类取结果
	def get_list_search_column(self,html,tid):
		videos=[]
		temporary=self. get_lineList(Txt=html,mark=r'<div class="ind01" id="ind01',after=r'缩略图')
		for vod in temporary:
			url=self.get_RegexGetText(Text=vod,RegexText=r'lanmu3="video_recomm"><a href="(https{0,1}://tv\..+?\.shtml)"',Index=1)
			if url=='':
				continue
			title=self.removeHtml(txt=self.get_RegexGetText(Text=vod,RegexText=r'id="video_playlist_cont_\d+?">(.+?)</a>',Index=1))
			img=self.get_RegexGetText(Text=vod,RegexText=r'src="(.+?)"',Index=1)
			if len(url) == 0:
				continue
			vod_id="{0}###{1}###{2}###{3}".format(tid,title,url,img)
			# print(vod_id)
			videos.append({
				"vod_id":vod_id,
				"vod_name":title,
...
# l=T.searchContent(key='军事科技',quick='')
# # l=T.categoryContent(tid='特别节目',pg='1',filter=False,extend={})
# # for x in l['list']:
# # 	print(x['vod_id'])
